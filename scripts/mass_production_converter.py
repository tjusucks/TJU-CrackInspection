...
def run_mass_conversion():
    ...
    if not all_imgs:
        print(f"Photos not found in {RAW_IMG_DIR}, please check the path.")
        return
    ...
    for phase, img_list in splits.items():
        print(f"\nProcessing {phase} set (total {len(img_list)} images)...")
    ...
    print(f"\n✅ Mass conversion task completed. Data is ready: {OUTPUT_DIR}")
